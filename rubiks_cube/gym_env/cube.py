...
# Based on the following code: https://github.com/bellerb/RubiksCube_Solver


class RubiksCube:
    """
    Class containing the Rubik's Cube code
    """
    ...
    def __init__(
        ...
        Input: n - integer representing the width and height of the rubiks cube
               colours - list containing the first letter of ever colour you wish to use (Default = [0, 1, 2, 3, 4, 5]) [OPTIONAL]
               state - string representing the current state of the rubix cube (Default = None) [OPTIONAL]
        Description: Initialize the Rubik's Cube
        Output: None
        """
        if state is None:
            self.n = n
    ...
    def reset(self):
        ...
        """
        self.cube = [
            [[c for x in range(self.n)] for y in range(self.n)] for c in self.colours
        ]
    ...
    def flatten(self):
        """
        Input: None
        Description: Create list representation of the current state of the cube
        Output: list representing the cube current state
        """
        return [i for r in self.cube for s in r for i in s]
    ...
    def shuffle(self, l_rot=5, u_rot=100):
        ...
        Description: Shuffles Rubik's Cube to random solvable state
        Output: None
        """
    ...
    def show(self):
        """
        Input: None
        Description: Show the Rubik's Cube
        Output: None
        """
    ...
    def horizontal_twist(self, row, direction):
        ...
        Description: Twist desired row of Rubik's Cube
        Output: None
        """
        if row < len(self.cube[0]):
            if direction == 0:  # Twist left
                ...
                print("ERROR - direction must be 0 (left) or 1 (right)")
                return
            # Rotating connected face
            if direction == 0:  # Twist left
                if row == 0:
                    self.cube[0] = [
                        list(x) for x in zip(*reversed(self.cube[0]))
                    ]  # Transpose top
                elif row == len(self.cube[0]) - 1:
                    self.cube[5] = [
                        list(x) for x in zip(*reversed(self.cube[5]))
                    ]  # Transpose bottom
            elif direction == 1:  # Twist right
                if row == 0:
                    self.cube[0] = [list(x) for x in zip(*self.cube[0])][
                        ::-1
                    ]  # Transpose top
                elif row == len(self.cube[0]) - 1:
                    self.cube[5] = [list(x) for x in zip(*self.cube[5])][
                        ::-1
                    ]  # Transpose bottom
        else:
            print(
    ...
    def vertical_twist(self, column, direction):
        ...
        Description: Twist desired column of Rubik's Cube
        Output: None
        """
        if column < len(self.cube[0]):
            for i in range(len(self.cube[0])):
                if direction == 0:  # Twist down
                    ...
                    print("ERROR - direction must be 0 (down) or 1 (up)")
                    return
            # Rotating connected face
            if direction == 0:  # Twist down
                if column == 0:
                    self.cube[1] = [list(x) for x in zip(*self.cube[1])][
                        ::-1
                    ]  # Transpose left
                elif column == len(self.cube[0]) - 1:
                    self.cube[3] = [list(x) for x in zip(*self.cube[3])][
                        ::-1
                    ]  # Transpose right
            elif direction == 1:  # Twist up
                if column == 0:
                    self.cube[1] = [
                        list(x) for x in zip(*reversed(self.cube[1]))
                    ]  # Transpose left
                elif column == len(self.cube[0]) - 1:
                    self.cube[3] = [
                        list(x) for x in zip(*reversed(self.cube[3]))
                    ]  # Transpose right
        else:
            print(
    ...
    def side_twist(self, column, direction):
        ...
        Input: column - integer representing which column you would like to twist
               direction - boolean representing if you want to twist up or down [down - 0, up - 1]
        Description: Twist desired side column of Rubik's Cube
        Output: None
        """
        if column < len(self.cube[0]):
            for i in range(len(self.cube[0])):
            ...
            if direction == 0:  # Twist down
                if column == 0:
                    self.cube[4] = [
                        list(x) for x in zip(*reversed(self.cube[4]))
                    ]  # Transpose back
                elif column == len(self.cube[0]) - 1:
                    self.cube[2] = [
                        list(x) for x in zip(*reversed(self.cube[2]))
                    ]  # Transpose top
            elif direction == 1:  # Twist up
                if column == 0:
                    self.cube[4] = [list(x) for x in zip(*self.cube[4])][
                        ::-1
                    ]  # Transpose back
                elif column == len(self.cube[0]) - 1:
                    self.cube[2] = [list(x) for x in zip(*self.cube[2])][
                        ::-1
                    ]  # Transpose top
        else:
            print(
                f"ERROR - desired column outside of Rubik's cube range. Please select a column between 0-{len(self.cube[0])-1}"
            )
            return
